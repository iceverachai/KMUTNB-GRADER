...
const emitonAddUserBtnClick = defineEmits(["onAddUserBtnClick"])
const route = useRouter();
const PasswordCheck =reactive<{IsCheck:boolean}>( {IsCheck:false} )
...
const onCreateBtnClick = async (e: Event) => {
    e.preventDefault();
    if(userData.Password!==userData.ConfirmPassword){
        PasswordCheck.IsCheck = true;
        return;
    }
    ...
    try {
        ...
        const response = await fetch(import.meta.env.VITE_api + "/api/sentry/regist", {
            method: "POST",
            headers: {
        ...
        if (json.status === "success" && json.response) {
            emitonAddUserBtnClick("onAddUserBtnClick")
            route.go(0)
        } else {
...
<template>
    <div class="Container">
        <div class="icon-wrap">
            <div class="exit-button" @click="$emit('onAddAssingBtnClick')">
            ...
            <div class="controller">
                <form class="formWrap"  @submit="onCreateBtnClick">
                    <div :class="`inputWrapper ${state.Usernamefocus || (userData.Username !== '') ? 'focus' : ''}`">
                        <label class="placeholder">Username</label>
                        <input v-model="userData.Username" type="text" class="inputbox" @focus="() => state.Usernamefocus = true"
                            @blur="() => state.Usernamefocus = false" />
                    </div>
                    <div :class="`inputWrapper ${state.Firstnamefocus || (userData.Firstname !== '') ? 'focus' : ''}`">
                        <label class="placeholder">Firstname</label>
                        <input v-model="userData.Firstname" type="text" class="inputbox" @focus="() => state.Firstnamefocus = true"
                            @blur="() => state.Firstnamefocus = false" />
                    </div>
                    <div :class="`inputWrapper ${state.Lastnamefocus || (userData.Lastname !== '') ? 'focus' : ''}`">
                        <label class="placeholder">Lastname</label>
                        <input v-model="userData.Lastname" type="text" class="inputbox" @focus="() => state.Lastnamefocus = true"
                            @blur="() => state.Lastnamefocus = false" />
                    </div>
                    <div :class="`inputWrapper ${state.Passwordfocus || (userData.Password !== '') ? 'focus' : ''}`">
                        <label class="placeholder">Password</label>
                        <input v-model="userData.Password" type="password" class="inputbox" @focus="() => state.Passwordfocus = true"
                            @blur="() => state.Passwordfocus = false" />
                    </div>
                    <div :class="`inputWrapper ${state.ConfirmPasswordfocus || (userData.ConfirmPassword !== '') ? 'focus' : ''}`">
                        <label class="placeholder">Confirm Password</label>
                        <input v-model="userData.ConfirmPassword" type="password" class="inputbox" @focus="() => state.ConfirmPasswordfocus = true"
                            @blur="() => state.ConfirmPasswordfocus = false" />
                    </div>
                    <div v-if="PasswordCheck.IsCheck" style="color: red;">
